 ...
 * 「存在しない時刻」と「2回存在する時刻」に専用の扱いは設けない（過去のレビュー指摘の決定）。
 *
 ...
 * 失敗をどう扱うかが散らばる。ここへ集約する（過去のレビュー指摘）。
 ...
 * ようにする（過去のレビュー指摘）。「妥当と判定したのに変換で例外」という境界を
 * 画面に作らないためである。
 *
 ...
 * 適用状態が違うと、保存される瞬間がずれるためである（過去のレビュー指摘）。
 *
 ...
 * 区間履歴と変更履歴を取り違える読み方を招く（過去のレビュー指摘）。
 *
 * @param {string} iso
