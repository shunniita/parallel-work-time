...
> [!NOTE]
> **2026-08-06 追記: 2.1（S8-1）と 2.2（S8-2）は対応済み。** §6 に各指摘への回答を記録した。Vitest 1,182件・E2E 118件が成功している。§3・§4 の指摘のうち、3.3 と 4.3 は事実誤認または仕様どおりと判断した（根拠は §6.2）。残りは既存の記録（FB-9、C-10、13章の性能目標）へ紐づけた。
...
| 指摘 | 扱い |
| ---- | ---- |
| 3.1 集計済みの不変条件が一時的に破れる | S8-1 の修正で**外部への影響は塞いだ**（未確定のまま転記済みにはできない）。ただし「集計済みのまま未終了区間を持つ」状態自体は残る。解消するには編集時の自動差し戻しが要るが、仕様書7.1 は集計済み→作業中を「確認操作を行う」と定めており、自動化は仕様変更にあたる。§6.4 の判断待ちとする |
...
**3.1 の残り: 集計済みのまま未終了区間を持つ状態をどう扱うか。**

S8-1 の修正で「未確定のものを転記済みにする」経路は塞がれ、画面も転記済みボタンを無効にして理由を示す。残るのは「状態バッジは集計済みだが、中身は未確定」という表示上の食い違いである。

| 案 | 内容 | 影響 |
| -- | ---- | ---- |
| A（現状） | 遷移の入口で守る。集計済みのまま未確定を許す | 仕様変更なし。集計画面で未確定と分かる |
| B | 集計済みで区間を開始したら自動で作業中へ戻す | 仕様書7.1 の「集計済み→作業中は確認操作」と食い違う。仕様の改訂が要る |
| C | 集計済みでは区間の開始を拒む | 仕様書7.2 が閲覧のみとするのは転記済みとアーカイブだけ。こちらも改訂が要る |

現状は A である。B・C はいずれも仕様書の改訂を伴うため、Step 8 の範囲では変更しない。
