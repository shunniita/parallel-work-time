...
Step 6 着手前の分として次の13件へ対応した。残る16件は各 Step の実装時に扱う。
...
| E（テスト・開発体験） | E-20、E-22、E-24 | E-21、E-23 |
...
- 残り: UI 層の単体テストとカバレッジ計測手段。カバレッジは `@vitest/coverage-v8` の追加を伴うため、`test:coverage` スクリプトとあわせてここで扱う。
