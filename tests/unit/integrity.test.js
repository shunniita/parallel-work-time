...
describe('validateImport（構造＋業務整合性）', () => {
  ...
  it('前後空白を除いた案件IDの重複も重複として扱う', () => {
    // 構造検証は非正規な文字列を先に弾くため（過去のレビュー指摘）、この経路は
  ...
  it('同一実施回に同じ作業項目定義が2行あると拒否する（過去のレビュー指摘）', () => {
...
describe('参加者の意味を通常入力と一致させる（過去のレビュー指摘）', () => {
  ...
  it('文字列以外の要素が混じっても、同じ一覧の重複を見落とさない（過去のレビュー指摘）', () => {
