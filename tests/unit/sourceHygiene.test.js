 ...
 *
 * `directEntryOps.js` の文字列リテラルへ NUL バイトが1個入り込み、
 * Git がファイルをバイナリと判定して差分を出さなくなっていた（過去のレビュー指摘）。NUL はエディタ上でも空白に見え、テストも普通に通るため、混入しても
 * 誰も気づけない。機械的に見張る。
 *
 * 対象は配布物に入るファイル（`index.html` と `src/`、過去の実装計画）に絞る。
 */
