 ...
 * インラインHTMLのタグを取り除く。
 *
 * 変化しなくなるまで繰り返す。現在の `[^>]+` は内側の `<` ごと飲み込むため
 * 1回で不変点に達するが、それはこの書き方に依存している。範囲を狭めた途端に
 * 「除去した結果が新しいタグを組み立てる」入力を取りこぼすため、回数ではなく
 * 不変点を条件にする。
 *
 * ここが担うのは断片の質であって、出力の安全ではない。断片へ記号を持ち出さない
 * ことは最後の絞り込みが保証する（{@link headingSlug}）。
 */
